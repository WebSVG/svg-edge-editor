...
class Geometry{
    ...
    angle_v2v(v1,v2){
        const aa = Vector.angle({x:1,y:0},v1)
        const ab = Vector.angle({x:1,y:0},v2)
        return ab - aa
    }
    
    angle_v(v1){
        return Vector.angle({x:1,y:0},v1)
    }
    angle_edge_point(edge,point){
        const vect1 = Vector.sub(edge.v2,edge.v1)
        const vect2 = Vector.sub(point,edge.v1)
        return this.angle_v2v(Vector.normalise(vect1),Vector.normalise(vect2))
    }
    angle_edge_point_(edge,point){
        const a1 = Vector.angle(edge.v1,edge.v2)
        const a2 = Vector.angle(edge.v1,point)
        return a2 - a1
    }
}
