...
function get_c(d){
    ...
    for(let i=0;i<d.length;i++){
        if((d[i] == 'c')&&(i <d.length)){// search for 'c'
            for(let j=i+1;j<d.length;j++){// then begin from next
                const entry = d[j]
                if(isNaN(parseFloat(entry))){           //return as soon as Non a number as 'c' is over
                    return result
                }
                result.push(entry)
            }
    ...
    if(result.length == 0){
        console.error("no 'c' instruction found in the 'd' attribute of SVG 'path' ")
    }
    return result       //return also if 'c' continued till the end
}

function get_all_points_list(d){
    let text_p_list = d.split(/[\s,]+/)
    text_p_list = get_c(text_p_list)
    let result = []
...
class EdgeStyle{
    ...
    curved_line(e,id=0,xscale=1,yscale=1){
        ...
        const style_id = id-1
        const TargetVector = Vector.sub(e.v2,e.v1)
        //console.log(" ---------------------- ")
        //console.log(`input = (${e.v1.x},${e.v1.y})->(${e.v2.x},${e.v2.y})`)
        //console.log(`target = (${TargetVector.x},${TargetVector.y}) angle = '${angle_v(TargetVector)}'`)
        const angle = geom.angle_v2v(this.vectors[style_id],TargetVector)
        ...
        let start = `M ${e.v1.x} ${e.v1.y} c `
        let middle = ""
        let count = 0
        this.p_all_lists[style_id].forEach((p)=>{
            let q = {x:p.x,y:p.y*yscale}
            let adjusted_point = Vector.rotate(q,angle)
        ...
        let end = `L ${e.v2.x} ${e.v2.y} `
        return start + middle + end
    }
    update_curves_list(item){
        console.log(` => EdgeStyle todo update_curves_list`)
        console.log(item)
        const path = item?.querySelector("svg")?.querySelector("g")?.querySelector("path")
        if(path){
            this.add_path_d(path.getAttribute("d"))
        }
        else{
